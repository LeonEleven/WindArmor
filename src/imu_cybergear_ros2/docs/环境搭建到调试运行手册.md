...
1. 可调最大速度已放宽到 `5.0 rad/s`（`manual_speed_max`）。
2. AUTO 模式下，电机目标变化速度受两项共同限制：
   - 电机速度参数（`0x7017`，由 `+/-` 修改）
   - 安全步进上限 `max_position_step`
3. AUTO 模式每步最大角度 = 速度 × `command_interval_sec`。
   - 例：1.2 rad/s × 0.02s = 0.024 rad ≈ 1.4°/步
   - 加大 `command_interval_sec` 或 `max_position_step` 可使响应更"猛"
