"""ALG-005 implementation-stage gates and frozen ALG-001–004 inheritance.

Inherited assertions retain the ALG-004 fixture thresholds and schedules.
The requested-intent adapter injects only the inherited request source; the
production ALG-004 shaper and ALG-005 final process path both execute.
"""
...
from windarmor_flight_control.algorithms.alg005_candidate_a import (
    Alg005CandidateACore, CandidateAConfiguration, RecoveryPhase,
)
from .alg005_benchmark import (
    SCRIPTED_SCENARIOS, SCRIPTED_BY_ID, DYNAMIC_SCENARIOS, DYNAMIC_BY_ID,
    DT, motion_state, run_scripted, run_dynamic, metrics as process_metrics,
)
...
FACTORY = (
    "windarmor_flight_control.algorithms.alg005_candidate_a:create_controller"
)
DEFAULT_FACTORY = (
...
FROZEN_CONFIGURATION = {
    ...
    "disturbance_pitch_rad": 0.040,
    "disturbance_pitch_rate_rad_s": 0.200,
    "settling_pitch_rad": 0.020,
    "settling_pitch_rate_rad_s": 0.100,
    "stable_pitch_rad": 0.010,
    "stable_pitch_rate_rad_s": 0.050,
    "stable_dwell_sec": 0.300,
    "recovery_timeout_sec": 3.000,
}
...
class _FinalRequestedIntentAdapter:
    """Use the real final process path with inactive (0,0) process context."""

    def __init__(self):
        self.core = _core()
        self.source = _RequestedSource()
        # Replace only ALG-003 requested intent, preserving both production
        # ALG-004 output shaping and the production ALG-005 process manager.
    ...
    @property
    def previous_shaped_intent(self):
        return self.core.observation.final_process_intent

    def update(self, request, dt):
        if (isinstance(request, bool) or not isinstance(request, (int, float))
                or not math.isfinite(float(request))):
            self.core.external_fail_close()
            raise ValueError("requested intent must be finite")
        self.source.request = request
        result = self.core.update(0.0, 0.0, dt)
        assert self.core.observation.phase == RecoveryPhase.STABLE
        assert not self.core.observation.episode_active
        return result


def _shaper():
    return _FinalRequestedIntentAdapter()


def _core():
    return Alg005CandidateACore(CandidateAConfiguration())


def _controller():
    return Alg005CandidateAController(MOTOR_NAMES, CandidateAConfiguration())
...
def test_factory_loads_frozen_default_and_explicit_configuration() -> None:
    ...
    assert isinstance(default, Alg005CandidateAController)
...
def test_default_and_teaching_controllers_remain_separate() -> None:
    ...
    assert isinstance(candidate, Alg005CandidateAController)
...
def _assert_ordinary_contract(samples, resets=()):
    previous = 0.0
    for index, sample in enumerate(samples):
        if index in resets:
            previous = 0.0
        if sample.phase == RecoveryPhase.TIMED_OUT:
            assert sample.u == 0.0
            _assert_safe_stop(sample.command)
            continue
        target = clamped_target(sample.requested)
        assert abs(sample.u) <= U + tolerance(abs(sample.u), U)
        delta = abs(sample.u - previous)
        assert delta <= S * sample.dt + tolerance(delta, S * sample.dt)
        assert min(previous, target) - tolerance(sample.u, min(previous, target)) <= sample.u
        assert sample.u <= max(previous, target) + tolerance(sample.u, max(previous, target))
        assert sample.command.motor_positions_rad == dict.fromkeys(MOTOR_NAMES, 0.0)
        assert sample.command.fan_commands.left == sample.command.fan_commands.right == 0.0
        assert not sample.command.request_safe_stop
        previous = sample.u


def _assert_process_accounting(samples, resets=()):
    """Independent interval-list oracle; no Candidate timer or phase reuse."""
    active, latched = False, False
    episode, dwell = [], []
    phase = RecoveryPhase.STABLE
    first_settling, entry = None, 0.0
    for index, sample in enumerate(samples):
        if index in resets:
            active, latched = False, False
            episode, dwell = [], []
            phase, first_settling, entry = RecoveryPhase.STABLE, None, 0.0
        previous_phase = phase
        if not latched:
            entering = not active and (abs(sample.theta) >= 0.040 or abs(sample.omega) >= 0.200)
            if entering:
                active, episode, dwell, first_settling = True, [], [], None
            if active:
                episode.append(sample.dt)
                if abs(sample.theta) <= 0.010 and abs(sample.omega) <= 0.050:
                    dwell.append(sample.dt)
                else:
                    dwell = []
                if math.fsum(episode) >= 3.000:
                    phase, latched = RecoveryPhase.TIMED_OUT, True
                elif entering:
                    phase = RecoveryPhase.DISTURBED
                elif math.fsum(dwell) >= 0.300:
                    phase, active = RecoveryPhase.STABLE, False
                elif abs(sample.theta) <= 0.020 and abs(sample.omega) <= 0.100:
                    phase = RecoveryPhase.SETTLING
                else:
                    phase = RecoveryPhase.RECOVERING
                if phase == RecoveryPhase.SETTLING and first_settling is None:
                    first_settling = math.fsum(episode)
                if entering or phase != previous_phase:
                    entry = math.fsum(episode)
            else:
                phase, episode, dwell = RecoveryPhase.STABLE, [], []
                first_settling, entry = None, 0.0
        observed = sample.observation
        assert observed.phase == phase
        assert observed.timeout_latched == latched
        assert observed.episode_active == active
        assert observed.episode_elapsed_sec == math.fsum(episode)
        assert observed.stable_dwell_sec == math.fsum(dwell)
        assert observed.first_settling_entry_elapsed_sec == first_settling
        assert observed.phase_entry_elapsed_sec == entry


@pytest.mark.parametrize("scenario", SCRIPTED_SCENARIOS, ids=lambda s: s.scenario_id)
def test_alg005_frozen_level_d_process_and_final_output(scenario):
    runs = [run_scripted(scenario) for _ in range(3)]
    assert runs[0] == runs[1] == runs[2]
    samples = runs[0]
    _assert_process_accounting(samples, scenario.reset_before)
    _assert_ordinary_contract(samples, scenario.reset_before)
    suffix = scenario.scenario_id[7:10]
    if suffix == "D00":
        assert all(s.phase == RecoveryPhase.STABLE for s in samples)
    elif suffix == "D01":
        assert [samples[i].phase for i in (14, 15, 16)] == [
            RecoveryPhase.STABLE, RecoveryPhase.DISTURBED, RecoveryPhase.RECOVERING]
    elif suffix == "D02":
        assert samples[-2].phase == RecoveryPhase.SETTLING
        assert samples[-2].observation.stable_dwell_sec == 0.280
        assert samples[-1].phase == RecoveryPhase.STABLE
        assert samples[-1].observation.stable_dwell_sec == 0.300
    elif suffix == "D03":
        assert [samples[i].phase for i in (20, 30, 35, 54)] == [
            RecoveryPhase.SETTLING, RecoveryPhase.RECOVERING,
            RecoveryPhase.SETTLING, RecoveryPhase.STABLE]
        assert samples[29].observation.stable_dwell_sec == 0.100
        assert samples[30].observation.stable_dwell_sec == 0.0
        assert samples[35].observation.first_settling_entry_elapsed_sec == samples[20].observation.first_settling_entry_elapsed_sec
        m = process_metrics(samples)
        assert m["recovery_time"] == pytest.approx(0.800, abs=ABS_TOL, rel=1e-6)
        assert m["settling_completion_time"] == pytest.approx(0.700, abs=ABS_TOL, rel=1e-6)
        assert m["final_settling_interval"] == pytest.approx(0.400, abs=ABS_TOL, rel=1e-6)
        assert m["settling_setback_count"] == 1
    elif suffix == "D04":
        assert samples[0].phase == RecoveryPhase.STABLE
        assert samples[1].phase == RecoveryPhase.DISTURBED
        assert samples[2].phase == RecoveryPhase.SETTLING
        assert samples[-2].phase != RecoveryPhase.STABLE
        assert samples[-1].phase == RecoveryPhase.STABLE
    elif suffix == "D90":
        assert samples[163].observation.episode_elapsed_sec == 2.980
        assert samples[163].phase != RecoveryPhase.TIMED_OUT
        assert samples[164].observation.episode_elapsed_sec == 3.000
        assert samples[164].phase == RecoveryPhase.TIMED_OUT
        assert all(s.observation == samples[164].observation for s in samples[165:])


@pytest.mark.parametrize("name", ["D01", "D02", "D03", "D04", "D90"])
def test_alg005_o01_mirror_repeatability_forward_reverse_order(name):
    controller = _controller()
    ids = [f"ALG005-{n}{sign}" for n in ("D01", "D02", "D03", "D04", "D90")
           for sign in ("P", "N")]
    reference = {sid: run_scripted(SCRIPTED_BY_ID[sid]) for sid in ids}
    for order in (ids, list(reversed(ids))):
        for _ in range(3):
            for sid in order:
                assert run_scripted(SCRIPTED_BY_ID[sid], controller) == reference[sid]
    p, n = reference[f"ALG005-{name}P"], reference[f"ALG005-{name}N"]
    for left, right in zip(p, n):
        assert left.theta == -right.theta
        assert left.omega == -right.omega
        assert left.u == -right.u
        assert replace(left.observation, final_process_intent=0.0) == replace(right.observation, final_process_intent=0.0)
    assert process_metrics(p) == process_metrics(n)


@pytest.mark.parametrize("scenario", DYNAMIC_SCENARIOS, ids=lambda s: s.scenario_id)
def test_alg005_level_e_frozen_hard_gates_and_three_complete_runs(scenario):
    runs = [run_dynamic(scenario) for _ in range(3)]
    assert len(runs[0]) == scenario.steps + scenario.pre_steps
    assert runs[0] == runs[1] == runs[2]
    samples, m = runs[0], process_metrics(runs[0])
    _assert_process_accounting(samples)
    _assert_ordinary_contract(samples)
    for sample in samples:
        for number in (sample.time, sample.theta, sample.omega, sample.u,
                       sample.requested, sample.alpha, sample.external_acceleration,
                       sample.observation.episode_elapsed_sec, sample.observation.stable_dwell_sec,
                       sample.observation.phase_entry_elapsed_sec):
            assert math.isfinite(number)
    for key, value in m.items():
        if key != "phase_timeline" and value is not None:
            assert math.isfinite(value)
    if "E00" in scenario.scenario_id:
        assert all(s.phase == RecoveryPhase.STABLE and s.u == 0.0
                   and s.theta == 0.0 and s.omega == 0.0 for s in samples)
        assert m["recovery_time"] is None
    elif "E90" in scenario.scenario_id:
        assert m["recovery_time"] is None
        assert 3.000 <= m["timeout_elapsed"] < 3.020
        assert samples[148].phase != RecoveryPhase.TIMED_OUT
        assert samples[149].phase == RecoveryPhase.TIMED_OUT
        assert all(s.observation == samples[149].observation for s in samples[150:])
        assert all(s.command == FlightCommand.safe_stop() and s.u == 0.0
                   for s in samples[149:])
    else:
        assert {RecoveryPhase.DISTURBED, RecoveryPhase.RECOVERING,
                RecoveryPhase.SETTLING, RecoveryPhase.STABLE} <= {s.phase for s in samples}
        assert all(s.phase != RecoveryPhase.TIMED_OUT for s in samples)
        assert m["recovery_time"] <= 2.500 + ABS_TOL
        assert m["settling_completion_time"] is not None
        assert m["final_settling_interval"] is not None
        assert all(s.phase == RecoveryPhase.STABLE and abs(s.theta) <= 0.010
                   and abs(s.omega) <= 0.050 for s in samples[-25:])
        assert m["effective_pitch_sign_reversal_count"] <= 2
        if any(name in scenario.scenario_id for name in ("E01", "E02", "E03")):
            assert m["overshoot_ratio"] <= 0.50 + ABS_TOL
        if "E04" in scenario.scenario_id:
            assert all(s.phase == RecoveryPhase.STABLE for s in samples[:25])
            assert samples[25].omega == scenario.impulse
            assert samples[25].phase == RecoveryPhase.DISTURBED
            assert m["phase_timeline"][0] == (0.520, "DISTURBED")


@pytest.mark.parametrize("name", ["E01", "E02", "E03", "E04", "E90"])
def test_alg005_level_e_framewise_mirror_all_observations_and_metrics(name):
    p = run_dynamic(DYNAMIC_BY_ID[f"ALG005-{name}P"])
    n = run_dynamic(DYNAMIC_BY_ID[f"ALG005-{name}N"])
    assert len(p) == len(n)
    for left, right in zip(p, n):
        for field in ("theta", "omega", "u", "requested", "alpha", "external_acceleration"):
            assert abs(getattr(left, field) + getattr(right, field)) <= tolerance(getattr(left, field), getattr(right, field))
        assert replace(left.observation, final_process_intent=0.0) == replace(right.observation, final_process_intent=0.0)
...
def test_level_e_runner_exact_step_order_semi_implicit_and_impulse():
    for scenario in DYNAMIC_SCENARIOS:
        samples = run_dynamic(scenario)
        for index, (left, right) in enumerate(zip(samples, samples[1:])):
            omega_next = left.omega + DT * left.alpha
            theta_next = left.theta + DT * omega_next
            assert right.theta == theta_next
            assert right.omega == omega_next + (scenario.impulse if index + 1 == scenario.pre_steps else 0.0)
            assert left.alpha == 1.0 * left.theta + 6.0 * left.u - 2.4 * left.omega + left.external_acceleration
            assert left.time == index * DT


@pytest.mark.parametrize("timed_out", [False, True], ids=["ALG005-R01", "ALG005-R02"])
def test_alg005_reset_all_history_and_observations_matches_fresh(timed_out):
    controller = _controller()
    if timed_out:
        run_scripted(SCRIPTED_BY_ID["ALG005-D90P"], controller)
    else:
        for p, r in SCRIPTED_BY_ID["ALG005-D02P"].inputs[:18]:
            controller.update(motion_state(p, r), DT)
    assert controller.observation.episode_elapsed_sec > 0.0
    controller.reset()
    fresh = _controller()
    assert controller.observation == fresh.observation
    for p, r in ((0.0, 0.0),) * 3 + ((0.025, -0.10), (0.040, 0.0)):
        state = motion_state(p, r)
        assert controller.update(state, DT) == fresh.update(state, DT)
        assert controller.observation == fresh.observation


@pytest.mark.parametrize("latched", [False, True])
@pytest.mark.parametrize("fault", ["invalid-imu", "stale-imu", "stale-aggregate",
    "motor-unobserved", "motor-stale", "motor-invalid", "motor-unhealthy",
    "estop-unknown", "estop-active", "dt-zero", "dt-negative", "dt-nan",
    "dt-positive-inf", "dt-negative-inf"])
def test_alg005_f01_i01_i04_external_fail_close_and_timeout_transient_fault(latched, fault):
    controller = _controller()
    if latched:
        run_scripted(SCRIPTED_BY_ID["ALG005-D90P"], controller)
    else:
    ...
    dts = {"dt-zero": 0.0, "dt-negative": -0.01, "dt-nan": math.nan,
           "dt-positive-inf": math.inf, "dt-negative-inf": -math.inf}
    state = motion_state(0.05, 0.20) if fault in dts else _controller_fault_state(fault)
    validate_flight_state(state, MOTOR_NAMES)
    _assert_safe_stop(controller.update(state, dts.get(fault, DT)))
    if latched:
        assert controller.observation == before
        _assert_safe_stop(controller.update(motion_state(0.0, 0.0), DT))
        assert controller.observation == before
        assert controller._core._inherited_core._output_shaper.previous_shaped_intent == 0.0
        assert controller._core._inherited_core._inherited_core._previous_pitch is None
    else:
        fresh = _controller()
        assert controller.observation == fresh.observation
        for p, r in ((0.025, -0.10), (0.040, 0.0), (0.009, 0.040)):
            valid = motion_state(p, r)
            assert controller.update(valid, DT) == fresh.update(valid, DT)
            assert controller.observation == fresh.observation


def test_timeout_latched_path_never_calls_ordinary_core(monkeypatch):
    controller = _controller()
    run_scripted(SCRIPTED_BY_ID["ALG005-D90P"], controller)
    def forbidden(*args):
        pytest.fail("ordinary core called while timeout latched")
    monkeypatch.setattr(controller._core._inherited_core, "update", forbidden)
    before = controller.observation
    for dt in (0.020, 0.037, 1.0):
        _assert_safe_stop(controller.update(motion_state(0.0, 0.0), dt))
        assert controller.observation == before


@pytest.mark.parametrize("field", ["relative_pitch_rad", "relative_pitch_rate_rad_s"])
@pytest.mark.parametrize("value", [None, math.nan, math.inf, -math.inf])
def test_timeout_validation_reject_does_not_call_or_clear_controller(field, value):
    controller = _controller()
    run_scripted(SCRIPTED_BY_ID["ALG005-D90P"], controller)
    before = controller.observation
    state = motion_state(0.0, 0.0)
    invalid = replace(state, imu=replace(state.imu, **{field: value}))
    with pytest.raises(FlightValidationError):
        validate_flight_state(invalid, MOTOR_NAMES)
    assert controller.observation == before
    _assert_safe_stop(controller.update(state, DT))
    controller.reset()
    assert controller.update(state, DT) == _controller().update(state, DT)


def test_stable_only_exit_resets_dwell_without_settling_exit():
    core = _core()
    core.update(0.040, 0.0, DT)
    for _ in range(5):
        core.update(0.010, 0.050, DT)
    first = core.observation.first_settling_entry_elapsed_sec
    assert core.observation.stable_dwell_sec == 0.100
    core.update(0.018, 0.080, DT)
    assert core.observation.phase == RecoveryPhase.SETTLING
    assert core.observation.stable_dwell_sec == 0.0
    assert core.observation.first_settling_entry_elapsed_sec == first
    for _ in range(14):
        core.update(0.010, 0.050, DT)
        assert core.observation.phase == RecoveryPhase.SETTLING
    core.update(0.010, 0.050, DT)
    assert core.observation.phase == RecoveryPhase.STABLE


def test_timeout_precedence_over_simultaneous_dwell_completion():
    core = _core()
    core.update(0.050, 0.200, 2.700)
    assert core.observation.phase == RecoveryPhase.DISTURBED
    core.update(0.0, 0.0, 0.300)
    assert core.observation.phase == RecoveryPhase.TIMED_OUT
    assert core.observation.final_process_intent == 0.0
    assert core.observation.stable_dwell_sec == 0.300


def test_metric_fixed_origins_survive_later_episode_and_final_confirmation():
    from .alg005_benchmark import ScriptedScenario
    inputs = SCRIPTED_BY_ID["ALG005-D02P"].inputs + ((0.0, 0.0),) * 3 + ((0.050, 0.200),) * 2 + ((0.009, 0.040),) * 15
    samples = run_scripted(ScriptedScenario("metric-origins", inputs))
    m = process_metrics(samples)
    assert m["recovery_time"] == math.fsum(s.dt for s in samples[15:])
    assert m["settling_completion_time"] == math.fsum(s.dt for s in samples[20:])
    assert m["final_settling_interval"] == math.fsum(s.dt for s in samples[-15:])
    assert m["recovery_time"] > samples[-1].observation.episode_elapsed_sec


@pytest.mark.parametrize("key", tuple(FROZEN_CONFIGURATION))
@pytest.mark.parametrize("value", [True, "0.1", None, math.nan, math.inf, -math.inf, -1.0])
def test_all_configuration_fields_reject_invalid_or_nonfrozen_values(key, value):
    with pytest.raises(ValueError):
        CandidateAConfiguration.from_mapping({key: value})


def test_configuration_and_observation_are_immutable():
    from dataclasses import FrozenInstanceError
    controller = _controller()
    with pytest.raises(FrozenInstanceError):
        controller.configuration.recovery_timeout_sec = 4.0
    with pytest.raises(FrozenInstanceError):
        controller.observation.timeout_latched = True


@pytest.mark.parametrize("latched", [False, True])
@pytest.mark.parametrize("field", ["pitch", "rate", "dt"])
@pytest.mark.parametrize("value", [None, True, "0.02", math.nan, math.inf, -math.inf])
def test_core_input_reject_clears_history_but_preserves_timeout(latched, field, value):
    core = _core()
    for _ in range(150 if latched else 3):
        core.update(0.05, 0.20, DT)
    before = core.observation
    kwargs = {"pitch": 0.0, "rate": 0.0, "dt": DT}
    kwargs[field] = value
    with pytest.raises(ValueError):
        core.update(kwargs["pitch"], kwargs["rate"], kwargs["dt"])
    if latched:
        assert core.observation == before
        assert core.update(0.0, 0.0, DT) == 0.0
        assert core.observation == before
    else:
        fresh = _core()
        assert core.observation == fresh.observation
        assert core.update(0.025, -0.10, DT) == fresh.update(0.025, -0.10, DT)
        assert core.observation == fresh.observation


@pytest.mark.parametrize("periods", [(0.020,) * 16, (0.037,) * 10,
                                      (0.020, 0.037) * 8])
def test_process_variable_dt_counts_current_interval_and_continuous_dwell(periods):
    core = _core()
    core.update(0.040, 0.0, 0.020)
    accepted = []
    completed = False
    for dt in periods:
        core.update(0.010, 0.050, dt)
        if not completed:
            accepted.append(dt)
            assert core.observation.episode_elapsed_sec == math.fsum([0.020] + accepted)
            assert core.observation.stable_dwell_sec == math.fsum(accepted)
            completed = math.fsum(accepted) >= 0.300
            assert (core.observation.phase == RecoveryPhase.STABLE) == completed
    assert completed
