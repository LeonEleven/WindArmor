...
def test_runtime_has_only_two_observation_publishers_and_no_clients_or_services() -> None:
    tree = ast.parse(NODE_PATH.read_text(encoding="utf-8"))
    ...
    } == {"FlightRuntimeStatus", "FlightCommandPreview"}
    assert not any(method_name(call) == "create_client" for call in calls)
    assert not any(method_name(call) == "create_service" for call in calls)
