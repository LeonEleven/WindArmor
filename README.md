...
姿态增益和三种风扇响应曲线。任务开始时 `master` HEAD 为 `aa22ea3`，与
`origin/master` 一致，位于 `v0.3.1` 后两个提交：`8673fb0` 完成风扇安全与
确定性加固，`aa22ea3` 完成电机命令提交一致性、初始化回滚和 lifecycle 资源
释放加固；两项均已提交并推送。

用户在上述加固后自行运行统一 launch，测试 MANUAL/AUTO 和风扇功能并报告基本
正常。当前电机配置集中校验、显式状态转换契约和废弃参数兼容规则完成纯软件
验证后，用户再次启动整个系统并完成 MANUAL/AUTO 功能实机测试。该记录是用户
报告的正常功能验证，不是消息乱序、SDO 写入失败、初始化中断、停止失败、资源
销毁失败或全部恢复路径的实机故障注入。三模式 `4.0 rad/s` 及 `1200 μs`、
`1400 μs` 等候选值的精确性能边界仍应按实际设备继续验证或标定。
